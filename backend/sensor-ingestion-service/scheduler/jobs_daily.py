...
logger = setup_logger(name="daily_job")
...
def fetch_daily_weather_for_all():
    users = load_users()
    logger.info("Fetching DAILY weather for all users/farms")
    for user in users:
        user_id = user["id"]
        ...
        for farm in farms:
            ...
            try:
                df = client.get_forecast(lat, lon, ForecastType.DAILY)
                ...
                publish_message(payload, routing_key="weather.daily")
                logger.info(
                    f"Published daily forecast for user={user_name} ({user_id}), "
                    f"farm={farm_name} ({farm_id})"
                )
            except Exception as e:
                logger.error(
                    f"Error fetching daily weather for user={user_name} ({user_id}), "
